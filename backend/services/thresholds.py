"""閾値ローダー — streamlit 依存なし"""
import yaml
from pathlib import Path
from functools import lru_cache

YAML_PATH = Path(__file__).parent.parent.parent / "thresholds.yaml"
...
def check_breach(value: float, cfg: dict) -> tuple:
    """
    Returns (level, icon)
...
def make_alert(level: str, message: str) -> dict:
    """アラート辞書を生成（streamlit 非依存）"""
